...
#define FFS_WIFI_MAX_APS_SUPPORTED              (40) /**< Maximum number of APs in scan list */
#define FFS_WIFI_MAX_SSID_LEN                   wificonfigMAX_SSID_LEN /**< Length of Wi-Fi SSID */

/**
 * @brief Wi-Fi scan results list
 */
typedef struct {
    WIFIScanResult_t        apInfo[FFS_WIFI_MAX_APS_SUPPORTED];
    uint8_t                 numAp;
    uint8_t                 valid;
 ...
 */
FFS_RESULT ffsWifiManagerLoadStaCredentials(const FfsUserContext_t *userContext, const WIFINetworkProfile_t *wifiCredentials);
...
/**
 * @brief Broadcast probe request with previous loaded SSID
 */
 ...
 */
FFS_RESULT ffsWifiManagerGetConnectionAttempt(const FfsUserContext_t *userContext, uint8_t index, WIFINetworkProfile_t *const attemptProfile, FFS_WIFI_CONNECTION_STATE *const connectionState, bool *const isUnderrun);
